...
const CSS = `
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .bg { position:absolute; inset:0; background-size:cover; background-position:center; filter:brightness(1.55) contrast(1.08); }
  .veil { position:absolute; inset:0; background:radial-gradient(ellipse at center, rgba(3,3,6,.28) 0%, rgba(3,3,6,.68) 100%); }
  ...
  h1 { font-family:'Cinzel',Georgia,serif; font-weight:900; color:#f2efe4; letter-spacing:.34em; text-indent:.34em;
       font-size:104px; text-shadow:0 0 70px rgba(224,30,55,.85), 0 4px 24px rgba(0,0,0,.9); }
  ...
  .sub { font-family:'PlexMono',monospace; font-size:22px; letter-spacing:.5em; text-indent:.5em; color:#b8b8c2; text-transform:uppercase; }
  .sub.red { color:#e01e37; }
  .foot { position:absolute; bottom:56px; left:0; right:0; text-align:center; font-family:'PlexMono',monospace;
          font-size:15px; letter-spacing:.42em; text-indent:.42em; color:#8a8a98; text-transform:uppercase; }
  .tag { position:absolute; top:52px; left:0; right:0; text-align:center; font-family:'PlexMono',monospace;
         font-size:16px; letter-spacing:.5em; text-indent:.5em; color:#e02038; text-transform:uppercase; }
  .end-title { font-family:'Cinzel',Georgia,serif; font-weight:900; color:#f2efe4; font-size:66px; letter-spacing:.34em; text-indent:.34em; text-shadow:0 0 50px rgba(224,30,55,.8); }
  .end-team { font-family:'Cinzel',Georgia,serif; font-size:30px; letter-spacing:.28em; text-indent:.28em; color:#b0b0bc; margin-top:14px; }
...
function card(bgFile, inner){
  const b64 = fs.readFileSync(path.join(R, bgFile)).toString('base64');
  return `<div class="card">
    <div class="bg" style="background-image:url('data:image/png;base64,${b64}')"></div>
